...
export async function GET() {
  try {
    ...
    return NextResponse.json({ status: "ok", service: "contracthunter", version: "0.1.8.1", database: "connected", ai: { enabled: config.AI_ENABLED, openAIConfigured: Boolean(config.OPENAI_API_KEY) }, tools: { git, npm, slither, aderyn, solcSelect } });
  } catch {
