...
import { getDatabase, getInvestigation, getScan, listHypothesesForInvestigation, listInvestigationFindings } from "@contracthunter/db";
...
export default async function InvestigationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hypotheses = listHypothesesForInvestigation(database, investigation.id);
  return <article className="article">
    ...
    {hypotheses.length > 0 && <section className="article-section"><h2>AI analysis available</h2>{hypotheses.map((item) => <p key={item.id}><Link className="muted-link" href={`/hypotheses/${item.id}`}>{item.title}</Link> · {item.reviewerId} · unverified</p>)}</section>}
