...
import { countHypothesesByInvariant, getDatabase, getInvariant, getProtocolAnalysis, listVulnerabilityHypotheses } from "@contracthunter/db";
...
export default async function InvariantPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const potentialViolations = countHypothesesByInvariant(database, invariant.id); const linked = listVulnerabilityHypotheses(database).filter((item) => parse<string[]>(item.violatedInvariantIds, []).includes(invariant.id));
  return <article className="article"><header className="page-head"><div><div className="eyebrow">Security invariant / {invariant.category}</div><h1>{invariant.title}</h1><p className="subhead">AI-proposed property from <Link className="muted-link" href={`/protocol-analyses/${invariant.analysisId}`}>{analysis?.protocolName ?? "protocol analysis"}</Link></p></div><SeverityBadge value={invariant.severityIfViolated} /></header>
    ...
    {linked.length > 0 && <section className="article-section"><h2>Suspected AI violations</h2>{linked.map((item) => <p key={item.id}><Link className="muted-link" href={`/hypotheses/${item.id}`}>{item.title}</Link> — unverified hypothesis</p>)}</section>}
  </article>;
}
