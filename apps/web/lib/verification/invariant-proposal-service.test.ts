...
const generated = (semantics: unknown) => ({ status: "generated", semantics, rationale: "The property maps to bounded current-state observations.", limitations: [], notPlannableReasons: [] });
...
describe("reviewed invariant proposal boundary", () => {
  ...
  it("preserves immutable generation history including not-plannable and provider failure", async () => {
    ...
    const second = await service(fake({ status: "not_plannable", semantics: null, rationale: "A bytes argument is required.", limitations: [], notPlannableReasons: ["unsupported_function_type"] })).generate(hypothesisId);
