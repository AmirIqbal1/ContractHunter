...
import { invariantProposalSchema, protocolAnalysisResultSchema, securityReviewResultSchema, verificationPlanProposalSchema, type AIProvider, type AIProviderResult, type InvariantProposalInput, type InvariantProposalProviderResult, type ProtocolAnalysisInput, type SecurityReviewInput, type SecurityReviewProviderResult, type VerificationPlanGenerationInput, type VerificationPlanProviderResult } from "@contracthunter/core";
...
export class OpenAIProvider implements AIProvider {
  ...
  async generateInvariantProposal(input: InvariantProposalInput): Promise<InvariantProposalProviderResult> {
    const started = Date.now();
    try {
      const response = await this.client.responses.parse({ model: input.model, store: false, tools: [], input: [{ role: "system", content: input.systemPrompt }, { role: "user", content: input.context.content }], text: { format: zodTextFormat(invariantProposalSchema, "invariant_proposal") } }, { timeout: input.timeoutMs });
      if (!response.output_parsed) throw new Error("OpenAI returned no structured invariant proposal.");
      return { proposal: response.output_parsed, actualModel: response.model ?? null, requestId: response._request_id ?? response.id ?? null, inputTokens: response.usage?.input_tokens ?? null, outputTokens: response.usage?.output_tokens ?? null, totalTokens: response.usage?.total_tokens ?? null, durationMs: Date.now() - started };
    } catch (error) {
      if (error instanceof OpenAI.AuthenticationError) throw new Error("OpenAI authentication failed.");
      if (error instanceof OpenAI.APIConnectionTimeoutError) throw new Error(`OpenAI invariant proposal timed out after ${input.timeoutMs} ms.`);
      if (error instanceof OpenAI.BadRequestError) throw new Error("OpenAI rejected the structured invariant proposal request.");
      throw error;
    }
  }
}
