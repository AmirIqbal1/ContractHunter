...
export const invariantProposalSchema = z.object({
  status: z.enum(["generated", "not_plannable"]), semantics: invariantAIProposalSemanticsSchema.nullable(), rationale: shortText,
  ...
  if (proposal.status === "not_plannable" && (proposal.semantics !== null || !proposal.notPlannableReasons.length)) context.addIssue({ code: z.ZodIssueCode.custom, message: "Not-plannable proposal needs a reason and no semantics." });
});
...
export type InvariantProposalGenerationResult = {
  ...
  rationale: string | null; limitations: string[]; notPlannableReasons: string[]; failureCode: InvariantProposalFailureCode | null;
