...
export class InvariantReplayWorkerClient {
  ...
  async run(input: InvariantReplayWorkerInput): Promise<InvariantReplayWorkerResult> {
    ...
    try {
      ...
      request = invariantReplayWorkerRequestSchema.parse({ command: "execute-invariant-replay", replayRunId: input.replayRunId, workspaceId: path.basename(workspace), scanId: input.scanId, hypothesisId: input.hypothesisId, resolvedCommit: input.resolvedCommit, compilerVersion: input.compilerVersion, invariantPlanHash: input.invariantPlanHash, replayPlanHash: input.replayPlanHash, counterexampleHash: input.counterexampleHash, timeoutMs: REPLAY_TIMEOUT_MS, maxOutputBytes: REPLAY_MAX_OUTPUT_BYTES });
