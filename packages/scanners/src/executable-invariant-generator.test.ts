...
import { canonicalInvariantPlan, executableInvariantManifestSchema, executableInvariantPlanSchema, invariantPlanHash, INVARIANT_HARNESS_MANIFEST, type ExecutableInvariantPlan } from "@contracthunter/core";
...
describe("invariant workspace manifest", () => {
  it("writes byte-identical controlled workspaces for repeated input", async () => {
    ...
    expect(executableInvariantManifestSchema.parse(first.manifest)).toEqual(first.manifest);
    ...
    expect(executableInvariantManifestSchema.safeParse({ ...first.manifest, planHash: "b".repeat(64) }).success).toBe(false);
    expect(executableInvariantManifestSchema.safeParse({ ...first.manifest, foundry: { ...first.manifest.foundry, seed: `0x${"b".repeat(64)}` } }).success).toBe(false);
  });
  it("rejects manifest extensions and repository-controlled config", async () => {
    ...
    expect(executableInvariantManifestSchema.safeParse({ ...result.manifest, command: "forge test" }).success).toBe(false);
